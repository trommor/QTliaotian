    ...
    , ui(new Ui::MainWindow)
{
    ...
    // 初始化 stackedWidget
    //ui->stackedWidget->addWidget(new ChatPage());  // 联系人页面
    ...
    // 连接添加好友信号到槽函数
       if (addFriendWindow) {
           connect(addFriendWindow, SIGNAL(friendAdded(QString)), this, SLOT(onFriendAdded(QString)));
       }
    // 初始化联系人列表
...
void MainWindow::onFunctionItemClicked(QListWidgetItem *item) {
    ...
    if (functionName == "聊天"|| functionName == "联系人") {
        // 显示联系人列表
...
void MainWindow::onAddFriendButtonClicked()
{
    if (!addFriendWindow) {
        ...
        addFriendWindow->show();  // 显示添加好友窗口
}
// 添加好友槽函数
void MainWindow::onFriendAdded(const QString &friendName) {
    // 将好友名称添加到联系人列表
    int row = ui->tbwFriendList->rowCount();
    ui->tbwFriendList->insertRow(row);
    ui->tbwFriendList->setItem(row, 0, new QTableWidgetItem(friendName));
